...
import { jobsAPI } from "../../services/api"; // Import API
...
const getFallbackImage = (id, images) => {
  if (!images || Object.keys(images).length === 0)
    return "https://via.placeholder.com/400x300?text=Job";
  ...
  const index = Math.abs(hash) % Object.keys(images).length;
  return Object.values(images)[index]?.default;
};
...
const TrendingJobs = () => {
  ...
  useEffect(() => {
    const fetchTrending = async () => {
      setLoading(true);
      try {
        // Fetch jobs based on active category
        // In a real app, 'trending' might be a specific sort or flag
        const data = await jobsAPI.getAll({
          category: activeCategory,
          limit: 6, // We need to implement limit in backend, but frontend asks for it
        });

        // Check if data is paginated (object with jobs array) or flat array
        const jobsList = data.jobs || data;
        // Client side slice if backend ignores limit for now or just to be safe
        setJobs(Array.isArray(jobsList) ? jobsList.slice(0, 6) : []);
      } catch (err) {
          ...
          {loading ? (
            ...
            jobs.map((job) => (
              ...
              >
                ...
                <div className="relative aspect-[4/3] sm:h-[220px] md:h-[240px] overflow-hidden bg-slate-50 flex items-center justify-center">
                  {/* Use Company Logo if available, else placeholder */}
                  <img
                    src={
                      job.companyId?.logo
                        ? `http://localhost:5001${job.companyId.logo}`
                        : getFallbackImage(job.title, jobImages)
                    }
                    alt={job.title}
                ...
                <div className="p-5 sm:p-6 md:p-8 flex flex-col flex-grow">
                  ...
                  <h3 className="text-lg md:text-xl font-bold text-[#083e47] mb-4 md:mb-6 group-hover:text-[#5BBB7B] transition-colors cursor-pointer leading-tight line-clamp-2">
                    <Link to={`/job/${job._id || job.id}`}>{job.title}</Link>
                  </h3>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-6 md:mb-8 mt-auto">
                    <div className="flex items-baseline gap-1">
                      ...
                      <span className="text-xs md:text-sm text-slate-400">
                        ...
                        {(job.salaryType || "Yearly").toLowerCase()}
                      </span>
                    </div>
                    ...
                    <div className="flex gap-2">
                      {(job.tags || []).slice(0, 2).map((tag, idx) => (
                        <span
                          key={idx}
                          className={`px-2.5 py-1 rounded-lg text-[10px] md:text-[11px] font-bold uppercase tracking-wider bg-[#F2F2F2] text-[#083e47]`}
                        >
                          {tag}
                        </span>
                      ))}
                      ...
                      <div className="flex gap-0.5 mb-1">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star
                            key={s}
                            size={12}
                            fill={s <= 4 ? "#FFA800" : "none"} // Mock rating
                            className={
                              s <= 4 ? "text-[#FFA800]" : "text-slate-300"
                            }
                          />
                        ))}
                      </div>
