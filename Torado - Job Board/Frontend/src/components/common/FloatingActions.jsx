...
const FloatingActions = ({ onOpenDemos }) => {
  ...
  return (
    <div className="fixed right-0 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-2 md:gap-3 pointer-events-none">
      {actions.map((action) => (
        <button
          key={action.id}
          ...
          className={`
            ...
            flex flex-col items-center justify-center w-16 h-16 md:w-18 md:h-18
        ...
        >
          <action.icon
            size={18}
            strokeWidth={1.5}
            className="md:size-[22px] mb-0.5 md:mb-1 transition-transform duration-300 group-hover:-rotate-12"
          />
