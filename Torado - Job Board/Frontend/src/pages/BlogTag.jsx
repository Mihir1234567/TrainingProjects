...
const BlogTag = () => {
  const { tagId } = useParams(); // Using tagId as the tag string itself usually
  const [visibleBlogs, setVisibleBlogs] = useState(6);
  const [filteredBlogs, setFilteredBlogs] = useState([]);

  // Normalize tag for display (capitalize first letter if needed, or decodeURI)
  ...
  useEffect(() => {
    ...
    // Filter logic: Check if the tag exists in the blog's tags array
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900">
      ...
      <section className="bg-[#f0f5fa] py-20 md:py-24 text-center border-b border-slate-100">
        ...
        <div className="flex items-center justify-center gap-2 text-[15px] font-medium">
          ...
          <span className="text-slate-400">/</span>
          <span className="text-[#5BBB7B]">{tagName}</span>
        </div>
      </section>
              ...
              >
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-xs font-medium text-slate-500">
                    <span>{blog.date}</span>
                    <span>•</span>
                    <span className="text-[#5BBB7B]">{blog.category}</span>
                  </div>
                  ...
                  <div className="pt-2 text-sm font-medium text-slate-400">
                    ...
                    <Link
                      ...
                      className="text-[#002333] hover:text-[#5BBB7B] transition-colors"
                    >
                      {blog.author}
                    </Link>
                  </div>
