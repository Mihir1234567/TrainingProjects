...
const RatingStars = ({ rating }) => {
  ...
  return (
    <div className="flex items-center">
      ...
      {[...Array(emptyStars)].map((_, i) => renderStar("empty", `e-${i}`))}
        ...
        {filters.map((filter) => (
          <div key={filter.name} className="flex items-center">
            <input
              ...
              onChange={onChange}
              disabled={filter.count === 0 && !selected.includes(filter.name)}
            ...
            <label
              ...
              className={`ml-3 text-sm flex-1 ${
                filter.count === 0 && !selected.includes(filter.name)
                  ? "text-gray-400"
                  : "text-gray-700"
              }`}
            >
              {filter.name}
              <span className="text-gray-500 ml-1">({filter.count})</span>
            </label>
          </div>
        ))}
      </div>
...
const CollectionsBooks = () => {
  // 🌟 FIX: Fetch products dynamically
  const { products: fetchedProducts, loading } = useProducts({ limit: 1000 });
  const MOCK_PRODUCTS = fetchedProducts || [];
