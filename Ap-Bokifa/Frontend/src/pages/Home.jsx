...
const Home = ({ showCoupon }) => {
  ...
  // 2. Fetch Data
  const { products: highlightedProducts } = useProducts({ isHighlight: true });
  const { products: bestSellingProducts } = useProducts({
    currentBestselling: true,
  });
  const { products: halfPriceProducts } = useProducts({ isHalfPrice: true });
  const { products: picksProducts } = useProducts({ isPickForYou: true });
  const { products: featuredProducts } = useProducts({ isFeatured: true });
