...
const YouTubeModal = ({ videoUrl, onClose }) => {
  ...
  return (
    ...
    >
      ...
      {/* Video Container */}
      <div
        className="max-w-4xl max-h-[90vh] w-[95vw] flex items-center justify-center"
  ...
  const Breadcrumbs = () => (
    <nav className="text-sm py-4 max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 text-gray-500">
      <ol className="flex space-x-2">
        ...
        <span>/</span>
        <li className=" text-gray-700">A Prayer for Owen Meany </li>
      </ol>
    </nav>
  );
            ...
            <div className="flex flex-col lg:flex-row gap-12 bg-white rounded-lg">
              ...
              <div className="lg:w-1/2 flex flex-col items-center">
                ...
                <div className="w-full max-w-full aspect-auto relative select-none">
                  ...
                  >
                    {thumbnailUrls.map((url, index) => (
                      <div key={index} className="h-full relative">
                        ...
                        />
                        {/* YouTube Video Button - Appears for all images except the first one */}
                        {index > 0 && videoUrls[index] && (
                          <button
                            onClick={() => setVideoModalUrl(videoUrls[index])}
                            className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 
                                                                bg-gray-500 hover:bg-gray-600 text-white rounded-full p-4 
                                                            shadow-lg transition-all duration-300 hover:scale-110 z-20"
                  ...
                  {/* Navigation Overlay - Left Half (Previous) - Only show for first image (no video) */}
                  ...
                  {/* Navigation Overlay - Right Half (Next) - Only show for first image (no video) */}
                  {selectedImageIndex === 0 && (
                    <div
