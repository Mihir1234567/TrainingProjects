...
const SidebarProductGridCard = ({ item }) => {
  ...
  return (
    <div
      className="group bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-200 cursor-pointer"
        ...
        >
          {shouldRotate ? (
            ...
            <div className="relative w-full h-full rounded-full">
              ...
              {backItem &&
                backItem.id && ( // Ensure backItem and its id exist
                  <img
              ...
              {middleItem &&
                middleItem.id && ( // Ensure middleItem and its id exist
                  <img
                    key={`middle-${middleItem.id}`}
              ...
              {lastTopItem &&
                lastTopItem.id && ( // Ensure lastTopItem and its id exist
                  <img
                    key={`last-${lastTopItem.id}`}
              ...
              {topItem &&
                topItem.id && ( // Ensure topItem and its id exist
                  <img
                    key={`top-${topItem.id}`} // This key is crucial for React to re-render
              ...
              <div className="p-4 overflow-y-auto flex-grow">
                <div className="grid grid-cols-2 gap-4">
                  {viewedItems.map((item) => (
                    <SidebarProductGridCard key={item.id} item={item} />
                  ))}
                </div>
              </div>
            </motion.div>
